...
```{r}
best_worst_tbl <-
rm_best_worst_trds_func(data_tbl = bt_metrics$data_series,
                        trd_tbl = bt_metrics$data_trd,
                        orig_calcs = bt_metrics$calcs_strtgy,
                        n_trds =  3,
                        init_eqty = 10000,
                        incl_dividends = TRUE)
...
#          SPY.Drawdown = round(drawdn * 100, 2)) |>
...
highchart(type = "stock") |>
  ...
  hc_add_theme(hc_theme_hcrt()) |>
  hc_rangeSelector(selected = 0, buttons = list(
    list(type = 'year', count = 5, text = '5y'),
    list(type = 'year', count = 10, text = '10y'),
    list(type = 'year', count = 15, text = '15y'),
    list(type = 'all', text = 'All'))) |>
  hc_add_series(chart_data[,5], yAxis = 0, name = "Open Trade", 
                type = "scatter", color = "green", 
                marker = list(symbol = "triangle")) |>
  hc_add_series(chart_data[,6], yAxis = 0, name = "Open Trade", 
                type = "scatter", color = "red", 
                marker = list(symbol = "triangle-down")) |>
  hc_add_series(chart_data[,1:4], 
                id = 1, 
                dataGrouping = list(enabled = FALSE),
                color = "black", lineColor = "black",
                tooltip = list(pointFormat = paste0(
  ...
  hc_add_yAxis(nid = 1L, title = list(text = "Prices"), 
               relative = 3, type = "logarithmic") |>
  hc_add_series(chart_data[,7], yAxis = 1, name = "Equity", type = "area",
                color = "green",
                negativeColor = "red",
                threshold = 10000, opacity = 0.75) |>
  hc_add_series(chart_data[,9], yAxis = 1, name = "Buy&Hold",
                color = "blue", type = "line",
                tooltip = list(
                  pointFormatter = JS("function() { return '<tr><td style=\"color: ' + this.series.color + '\">' + this.series.name + ': </td>' + '<td style=\"text-align: right\"><b>' +  new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(this.y) + '</b></td></tr>'; }"))
                ) |>
  hc_add_yAxis(nid = 2L, title = list(text = "Equity"), relative = 1) |>
  hc_add_series(chart_data[,8], yAxis = 2, name = "Drawdown", 
                color = "darkred", type = "area",
                tooltip = list(pointFormat = paste0("{series.name}: {point.y}%"))
                ) |>
  hc_add_yAxis(nid = 3L, title = list(text = "Drawdown"), relative = 1,
