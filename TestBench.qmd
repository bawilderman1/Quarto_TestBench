...
chart_data2 <- bt_metrics$data_series |>
  ...
  rename(Date = dt, Open = open, High = high, Low = low,
         ...
         Drawdown = drawdn,
         #SPY.MACD = MACD,
         #MACD.Signal = Signal, MACD.Hist = Histogram
         ) |>
  select(Date, Open, High, Low, Close,
         Trade.Open, Trade.Close, Equity, Drawdown,
         BuyAndHold, #MACD, MACD.Signal, MACD.Hist,
         #Zero
         ) |>
  as.xts()
# view(chart_data2)
...
rm(macd_data)
chart_data2$Zero <- xts(rep(0, NROW(chart_data2)), index (chart_data2))
...
# Example: Calculate data for two moving averages using quantmod
# quantmod::SMA expects an xts object
chart_data2$MA_50 <- TTR::SMA(chart_data2$Close, n = 50)
chart_data2$MA_200 <- TTR::SMA(chart_data2$Close, n = 200)

# Define the configuration for the optional price series
# The user does NOT need to specify yAxis here
my_optional_price_series <- list(
  ma_50_series = list(
    data = chart_data2$MA_50, 
    name = "50 SMA", 
    type = "line", 
    color = "orange", 
    tooltip = list(pointFormatter = JS(currency_tooltip_js))
