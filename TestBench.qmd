...
if (incl_buynhold) {
  # *** TODO: handle unrealized net profit and roa for both bnh & trades ***
  bt_bnh_totals_data = bt_data |>
    summarise(
      bnh_net_profit = last(bnh_eqty) - first(bnh_eqty),
      bnh_ttl_roa_pct = sum(bnh_pct_chg)
    )
  
  bt_totals_data <- bt_totals_data |>
    add_column(
      bnh_net_profit = bt_bnh_totals_data$bnh_net_profit,
      bnh_ttl_roa_pct = bt_bnh_totals_data$bnh_ttl_roa_pct
    )
...
tribble(
  ...
  "ROA", "Summary Stats", ttl$ttl_roa_pct, NA, NA,
  ...
  fmt_percent(
    rows = RowName %in% c("Profitable", "ROA", "CAGR", "Time in Market", "Max Trade DD", "Max Cumul. DD"),
  ...
  fmt_currency(
    rows = RowName %in% c("Profit & Loss", "Avg P&L", "Max Amount"),
    columns = c("Totals", "Winners", "Losers"),
    accounting = TRUE
  ) |>
  sub_values(fn = \(x) is.na(x), replacement = "")
  
rm(ttl, wnr, lsr, st_dt, end_dt)
```
