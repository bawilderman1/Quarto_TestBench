...
strtgy_entry <- \(.) {
 ifelse(.$rn > 1 & 
        (.$oc2 > .$UltimateSmoother & lag(.$oc2) <= lag(.$UltimateSmoother)), 
        1, 0)
}
strtgy_exit <- \(.) {
 ifelse(.$rn > 1 & 
...
### Backtest - Stock Chart

#### 1. Transform Chart Data

```{r}
# library(TTR)
# sourceCpp("src/macd.cpp")
sourceCpp("src/ultimate_smoother.cpp")
source("src/stock_chart_utilities.R")
js_tooltips <- load_js_tooltips()

# macd_tbl <- bt_metrics$data_series |>
#   mutate(Date = dt,
#          macdTbl(close, 12, 26, 9)) |>
#   select(Date, MACD, Signal, Zero)

# ma_tbl <- bt_metrics$data_series |>
#   mutate(Date = dt,
#          MA24 = TTR::SMA(close, n = 24),
#          MA48 = TTR::SMA(close, n = 48)) |>
#   select(Date, MA24, MA48)

ma_tbl <- bt_metrics$data_series |>
  mutate(Date = dt,
         UltimateSmoother = ultimateSmootherTbl((open + close) / 2, 10, 1)) |>
  select(Date, UltimateSmoother)

bt_xts <-transform_bt_series_to_xts(bt_metrics$data_series)

# bt_xts <- merge_indicators_into_xts(bt_xts, list(
#   macd = macd_tbl,
#   moving_averages = ma_tbl
# ))
...
#### 2. Generate Series Configs

```{r}
# macd_config <- generate_technical_series_config(
#   chart_data2[, c("MACD", "Signal", "Zero")],
#   yAxis = 0L,
#   colors = list(MACD = "cyan", Signal = "black", Zero = "gray"),
#   tooltip_js = list(
#     Zero = list(enabled = FALSE, pointFormat = NULL)
#   )
# )

# ma_config <- generate_technical_series_config(
#   chart_data2[, c("MA24", "MA48")],
#   yAxis = 1L,
#   colors = list(MA24 = "orange", MA48 = "purple"),
#   tooltip_js = list(
#     MA24 = js_tooltips$currency_tooltip_js,
#     MA48 = js_tooltips$currency_tooltip_js
#   )
# )

ma_config <- generate_technical_series_config(
  bt_xts[, c("UltimateSmoother")],
  yAxis = 1L,
  colors = list(UltimateSmoother = "orange"),
  tooltip_js = list(
    UltimateSmoother = js_tooltips$currency_tooltip_js
  )
)
```
...
```{r}
# build_stock_chart(
#   chart_data = add_blank_bars_xts(bt_xts, n = 3),
#   chart_title = "Backtest with Indicators",
#   technical_indicator_config = macd_config,
#   optional_price_series_config = ma_config
# )

build_stock_chart(
  chart_data = add_blank_bars_xts(bt_xts, n = 3),
  chart_title = "UltimateSmoother(10) Backtest",,
  optional_price_series_config = ma_config
)
```
...
```{r}
source("src/scatter_chart_utilities.R")
...
build_scatter_chart(
  ...
  y_col = "max_ur_drawup",
  group_col = "trd_rslt",
  chart_title = "Backtest - PnL vs. Max Unrealized Gain",
  y_axis_title = "Max Unrealized Gain",
  y_axis_tooltip = "Max Gain",
  chart_theme = hc_theme_hcrt()
)
```
...
stock_data <- spy_daily %>%  #stock_data %>%
  mutate(date = dt,
         macdTbl(close, 12, 26, 9),
         ultimateSmootherTbl((open + close) / 2, 10, 1))
...
ggplot(stock_data %>% slice((n()-150) : (n()-100)) 
       , aes(x = date, y = close)) +
  geom_candle(
    x = date, o = open, h = high, l = low, c = close,
    color = "black"
  ) +
  geom_line(aes(y = UltimateSmoother, color = "blue")) +
  theme_bw() +
  scale_color_manual(values = c("black", "orange")) +
  labs(title = "Ultimate Smoother")
