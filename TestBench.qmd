                 ...
                 incl_buynhold = FALSE)
{
  ...
  tbl = tbl |>
    ...
    mutate(is_closed = max(trd_exit),
           ...
           trd_drawup = round(log(eqty / slide_min(eqty, before=Inf)), 4),
           trd_ur_drawdn = round(log(eqty_l / slide_max(eqty_h, before=Inf)), 4),
           trd_ur_drawup = round(log(eqty_h / slide_min(eqty_l, before=Inf)), 4),
...
hchart(bt_metrics$data_trd |> 
  ...
  hc_tooltip(useHTML = TRUE, pointFormat = tltip, title = "foobar")
```
...
hchart(bt_metrics$data_trd, 
       ...
       hcaes(x = trd_pnl_pct, 
             ...
             group = trd_rslt)) |>
  hc_add_theme(hc_theme_hcrt()) |>
  hc_title(text = "Backtest - PnL vs. Max Unrealized Value") |>
  hc_xAxis(title = list(text = "Trade Profit and Loss"),
           plotLines = list(
             list(dashStyle = "Dash", value = 0, color = "black", zIndex = 3)
           )) |>
