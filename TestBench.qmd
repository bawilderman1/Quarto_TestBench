...
highchart(type = "stock") |>
  ...
  hc_rangeSelector(selected = 1, buttons = list(
    ...
    list(type = 'year', count = 15, text = '15y'),
    list(type = 'all', text = 'All'))) |>
  hc_add_series(data = (chart_data |> select(Date, SPY.MACD) |> na.omit()), 
                hcaes(x = Date, y = SPY.MACD), yAxis = 0, name = "MACD",
                type = "line", color = "cyan") |>
  hc_add_series(data = (chart_data |> select(Date, SPY.MACD.Signal) |> na.omit()), 
                hcaes(x = Date, y = SPY.MACD.Signal), yAxis = 0, name = "Signal",
...
#### 1. Define Indicators (from tibble or xts)

```{r}
source("src/chart_utilities.R")
library(TTR)
sourceCpp("src/macd.cpp")
...
#          Trade.Open, Trade.Close, Equity, Drawdown, BuyAndHold) |>
#   as.xts()
chart_data2 <- merge_indicators_into_xts(chart_data2, list(
  macd = macd_tbl,
  moving_averages = ma_tbl
))
```

#### 3. Generate Series Configs
...
ma_config <- generate_technical_series_config(
  ...
  yAxis = 1L,  # This is the standard price axis in your chart setup
