...
bt_tst <- result |>
  ...
  mutate(rn = row_number(),
         mth_nbr = month(dt),
         trd_entry = ifelse((mth_nbr == 10 | (mth_nbr == 1 & rn == 1)), 1, 0),
         ...
         eqty = 0) |>
  select(
    dt, open, close, rn, mth_nbr, 
    trd_entry, trd_exit, pos_size, pos_val, cash, eqty)
```
...
bt_tst2 |>
  select(-c(pos_size, pos_val, cash, eqty)) |>
...
rm(trd_entry_vec, trd_exit_vec, pos_size_vec, pos_val_vec,
   cash_vec, eqty_vec, open_vec, close_vec)
```
