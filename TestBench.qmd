...
# trd_subset <- \(tbl, cnt = 1, asc = TRUE)
#   tbl %>%
#     filter(trd_rslt != "Open") %>%
#     {`if`(asc, slice_max(., trd_pnl_pct, n = -cnt), slice_min(., trd_pnl_pct, n = -cnt))} %>%
#     arrange(trd_num)
  
# Hack: Not working***
bt_trd_data |> trd_subset(cnt = 3, asc = FALSE)
# asc <- TRUE
# bt_trd_data |>
#   filter(trd_rslt != "Open") |>
#   slice_max(trd_pnl_pct, n = -3) |>
#   # slice_min(trd_pnl_pct, n = -3) |>
#   arrange(trd_num)
```
