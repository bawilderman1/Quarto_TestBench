...
bt_strtgy <- result |>
  ...
  add_trades_func(strtgy_entry, strtgy_exit)
```
...
bt_totals_func <- \(tbl) {
  tbl = tbl |>
    ...
    summarise(summary = "Totals",
              ...
              ttl_roa_pct = exp(sum(bt_trd_data$trd_pnl_pct)) - 1,
...
bt_data2 <- bt_strtgy |>
  bt_sim_func(init_eqty = init_eqty,
              incl_dividends = incl_dividends,
              incl_buynhold = incl_buynhold)

bt_trd_data2 <- bt_data2 |> bt_trds_func()

bt_winloss_data2 <- bt_trd_data2 |> bt_winloss_func()

bt_totals_data2 <- bt_trd_data2 |> bt_totals_func()

bt_unrlzd_data2 <- bt_data2 |> bt_unrlzd_func(init_eqty, incl_buynhold)

bt_yrly_data2 <- bt_data2 |> bt_yrly_func(init_eqty, incl_buynhold)

calcs_strtgy2 <- calcs_strtgy_func(
  tbl = bt_data2,
  yrly_tbl = bt_yrly_data2,
  trds_tbl = bt_trd_data2)

if (incl_buynhold) {
  calcs_bnh2 <- calcs_bnh_func(
    tbl = bt_data2,
    yrly_tbl = bt_yrly_data2)
}

bt_metrics2 <- list(
  data_series = bt_data2,
  data_trd = bt_trd_data2,
  data_yrly = bt_yrly_data2,
  data_winloss = bt_winloss_data2,
  data_totals = bt_totals_data2,
  data_unrlzd = bt_unrlzd_data2,
  calcs_strtgy = calcs_strtgy_func(
    tbl = bt_data2,
    yrly_tbl = bt_yrly_data2,
    trds_tbl = bt_trd_data2)
)
...
if (incl_buynhold) {
  bt_metrics2 <- append(bt_metrics2, list(
    calcs_bnh = calcs_bnh_func(
    tbl = bt_data2,
    yrly_tbl = bt_yrly_data2)
  ))
}
...
if (incl_dividends) {
 div_vec <- as.numeric(bt_tst2$dividend) 
}
if (incl_buynhold) {
  bnh_ps_vec <- b <- vector("numeric" , length(bt_tst2$dt))
  bnh_eqty_vec <- b <- vector("numeric" , length(bt_tst2$dt))
}
...
if (incl_buynhold) {
  bt_bnh_unrlzd_data = bt_data |>
    summarise(
      bnh_net_profit = last(bnh_eqty) - init_eqty,
      bnh_ttl_roa_pct = exp(sum(bnh_pct_chg)) - 1
    )
  
  bt_unrlzd_data <- bt_unrlzd_data |>
    add_column(
      bnh_unrlzd_net_profit = bt_bnh_unrlzd_data$bnh_net_profit,
      bnh_unrlzd_ttl_roa_pct = bt_bnh_unrlzd_data$bnh_ttl_roa_pct
    )
}

rm(bt_bnh_unrlzd_data)
...
if (incl_buynhold) {
  ...
  bt_yrly_data <- bt_yrly_data |>
    add_column(
      bnh_st_eqty = bt_bnh_yrly_data$bnh_st_eqty,
      bnh_end_eqty = bt_bnh_yrly_data$bnh_end_eqty,
      bnh_max_yr_dd = bt_bnh_yrly_data$bnh_max_yr_dd
    )
...
if (incl_buynhold) {
  bnh_cagr_val <- round(
    cagr_func(
      dplyr::first(bt_yrly_data$bnh_st_eqty),
      dplyr::last(bt_yrly_data$bnh_end_eqty),
      nrow(bt_yrly_data)),
    4)
  ...
  bnh_mar_val <- round(
    mar_func(
      bnh_cagr_val,
      max_dd_func(bt_yrly_data$bnh_max_yr_dd)),
    3)
  
  bnh_calmar_val <- round(
    mar_func(
      cagr_func(
        dplyr::first(bt_yrly_tail_data$bnh_st_eqty),
        dplyr::last(bt_yrly_tail_data$bnh_end_eqty),
        nrow(bt_yrly_tail_data)),
      max_dd_func(bt_yrly_tail_data$bnh_max_yr_dd)),
    3)
  
  bnh_anlzd_sharp_val <- round(anlzd_sharp_func(bt_data$bnh_pct_chg, 12), 3)
}
```
...
if (incl_buynhold) {
  bt_metrics <- append(bt_metrics, list(
    calcs_bnh = list(
      cagr = bnh_cagr_val,
      mar = bnh_mar_val,
      calmar = bnh_calmar_val,
...
print(glue(
  ...
  pct = bt_metrics$calcs_strtgy$time_in_mkt))
print(glue(
  "CAGR: {scales::label_percent(0.01)(cagr)}",
  cagr = bt_metrics$calcs_strtgy$cagr))
print(glue("MAR: {bt_metrics$calcs_strtgy$mar}"))
print(glue("CALMAR: {bt_metrics$calcs_strtgy$calmar}"))
print(glue("Sharp: {bt_metrics$calcs_strtgy$sharp}"))
print(glue("Annualized Sharp: {bt_metrics$calcs_strtgy$anlzd_sharp}"))
if (incl_buynhold) {
  print(glue(
    "Buy & Hold CAGR: {scales::label_percent(0.01)(bnh_cagr)}",
    bnh_cagr = bt_metrics$calcs_bnh$cagr))
  print(glue("Buy & Hold MAR: {bt_metrics$calcs_strtgy$mar}"))
  print(glue("Buy & Hold CALMAR: {bt_metrics$calcs_bnh$calmar}"))
  print(glue("Buy & Hold Annualized Sharp: {bt_metrics$calcs_bnh$anlzd_sharp}"))
}
```
...
```{r}
wnr <- bt_metrics$data_winloss |> dplyr::filter(trd_rslt == "Winner")
lsr <- bt_metrics$data_winloss |> dplyr::filter(trd_rslt == "Loser")
ttl <- bt_metrics$data_totals
unr <- bt_metrics$data_unrlzd
st_dt <- dplyr::first(bt_metrics$data_series$dt)
end_dt <- dplyr::last(bt_metrics$data_series$dt)
...
bt_fmt_tbl <- tribble(
  ...
  "CAGR", "Summary Stats", bt_metrics$calcs_strtgy$cagr, NA, NA,
  "MAR", "Summary Stats", bt_metrics$calcs_strtgy$mar, NA, NA,
  "CALMAR", "Summary Stats", bt_metrics$calcs_strtgy$calmar, NA, NA,
  "Sharpe", "Summary Stats", bt_metrics$calcs_strtgy$sharp, NA, NA,
  "Ann. Sharpe", "Summary Stats", bt_metrics$calcs_strtgy$anlzd_sharp, NA, NA,
  "Total Months", "Misc. Stats", nrow(bt_metrics$data_series), NA, NA,
  "Trade Months", "Misc. Stats", nrow((bt_metrics$data_series) |> drop_na()), NA, NA,
  "Time in Market", "Misc. Stats", bt_metrics$calcs_strtgy$time_in_mkt, NA, NA,
  "Initial Equity", "Misc. Stats", init_eqty, NA, NA,
  "End Equity", "Misc. Stats", last(bt_metrics$data_series$eqty), NA, NA,
)
...
if (incl_buynhold) {
  bnh_list <- list(
    ...
    "Max Trade DD" = NA,
    "Max Cumul. DD" = min(bt_metrics$data_series$bnh_dd),
    "Profit Factor" = NA,
    "CAGR" = bt_metrics$calcs_bnh$cagr,
    "MAR" = bt_metrics$calcs_bnh$mar,
    "CALMAR" = bt_metrics$calcs_bnh$calmar,
    "Sharpe" = NA,
    "Ann. Sharpe" = bt_metrics$calcs_bnh$anlzd_sharp,
    "Total Months" = NA,
    "Trade Months" = NA,
    "Time in Market" = NA,
    "Initial Equity" = init_eqty,
    "End Equity" = last(bt_metrics$data_series$bnh_eqty)
  )
...
rm(ttl, wnr, lsr, unr, st_dt, end_dt, bt_fmt_tbl)

bt_gt
```

```{r}
## TODO: Add Optimized/Select Metrics (Remove Best & Worst Trade)
...
rm_trds_best <- as.numeric((bt_trd_data |> get_n_best_trades(3))$trd_num)
...
## TODO: Abstract base, yearly and calcs to subprocess functions
## TODO: Use trade start/end dates to remove from backtest and recalculate stats
```
...
highchart(type = "stock") |>
  ...
  hc_add_series(mnthly_spy_xts, 
                ...
                color = "forestgreen", lineColor = "forestgreen",
