...
create_series_config <- function(chart_data, technical_indicator_series = NULL) {
  series_list <- list()
  y_axis_offset <- 0L # Initialize offset

  if (!is.null(technical_indicator_series)) {
    # Shift yAxis indices for OHLC, Equity, Drawdown if technical indicator is present
    y_axis_offset <- 1L
...
chart_with_macd <- build_stock_chart(
  chart_data,
  "SPY - Backtest",
  hc_theme_hcrt(),
...
```{r}
# TODO: make optional price series tooltip currency
...
my_optional_price_series <- list(
  ma_50_series = list(data = chart_data$SPY.MA_50, name = "50-day MA", type = "line", color = "orange", tooltip = list(pointFormat = '<span style="color:{color}">●</span> {series.name}: <b>{point.y:.2f}</b><br/>')),
  ma_200_series = list(data = chart_data$SPY.MA_200, name = "200-day MA", type = "line", color = "purple", tooltip = list(pointFormat = '<span style="color:{color}">●</span> {series.name}: <b>{point.y:.2f}</b><br/>'))
)
